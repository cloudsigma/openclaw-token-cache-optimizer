...
test("agent id alone does not fabricate a conversation identity", () => {
	...
	assert.equal(resolve("/home/x/.openclaw/workspace", undefined, undefined, undefined, "main"), null)
})

test("workspace and agent scopes do not merge unrelated conversations", () => {
	const resolve = internals!.resolveSessionIdentity as Resolve
	assert.equal(resolve("/ws/a", undefined, undefined, undefined, "main"), null)
	assert.equal(resolve("/ws/b", undefined, undefined, undefined, "main"), null)
})

test("a native session id always takes precedence", () => {
	const resolve = internals!.resolveSessionIdentity as Resolve
	const r = resolve("/home/x/workspace", "agent:main:main", "stale-context", "trace-session", "main")
